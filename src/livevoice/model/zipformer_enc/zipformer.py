...
from ._compat import torch_autocast
...
class RelPositionMultiheadAttentionWeights(nn.Module):
    ...
    def forward(
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            pass
        # DISABLED (vendoring change): icefall logs attention entropy at 0.1% probability in
        # eval mode to monitor its own training. We only ever run this encoder frozen and in
        # eval, so the condition fires constantly across 16 layers and just spams the log.
        elif False and random.random() < 0.001 and not self.training:
